  ...
  // When there are same letters but not in the edges
    ...
    for (let i = 0; i <= towerBlocks.length - 1; i++) {
      ...
      for (remainingTBCombination of generateCombinations(remainingTowerBlocks)) {
        if (checkContiguous(towerBlocks[i] + remainingTBCombination)) {
          combinations.push(towerBlocks[i] + remainingTBCombination)
        }
        if (checkContiguous(remainingTBCombination + towerBlocks[i])) {
          combinations.push(remainingTBCombination + towerBlocks[i])
        }
      }
      dpDict[towerBlocks[i]]['combinations'] = combinations;
    }
    console.log('DP', dpDict);
  }
